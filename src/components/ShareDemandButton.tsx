...
export function ShareDemandButton({ demandId }: ShareDemandButtonProps) {
  ...
  return (
    <>
      <Button variant="outline" size="sm" onClick={() => setIsOpen(true)} className="flex-1 sm:flex-none">
              ...
              <div className="space-y-4">
                <div className="bg-muted/50 rounded-lg p-4 text-sm text-muted-foreground">
                  <ul className="space-y-2">
                    <li className="flex items-start gap-2"><Check className="h-4 w-4 text-green-600 flex-shrink-0 mt-0.5" />Qualquer pessoa com o link pode visualizar</li>
                    <li className="flex items-start gap-2"><Check className="h-4 w-4 text-green-600 flex-shrink-0 mt-0.5" />Inclui descrição, status e comentários</li>
                    <li className="flex items-start gap-2"><Check className="h-4 w-4 text-green-600 flex-shrink-0 mt-0.5" />Não é possível editar sem login</li>
                    <li className="flex items-start gap-2"><Check className="h-4 w-4 text-green-600 flex-shrink-0 mt-0.5" />Você pode revogar o link a qualquer momento</li>
                  </ul>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={expiration} onValueChange={(v) => setExpiration(v as ExpirationOption)}>
                    <SelectTrigger><SelectValue placeholder="Selecione a expiração" /></SelectTrigger>
                    <SelectContent>
                ...
                <Button onClick={handleCreateLink} disabled={createToken.isPending || (expiration === "custom" && !customDate)} className="w-full">
                  {createToken.isPending ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" />Criando...</>) : (<><Link className="mr-2 h-4 w-4" />Criar Link de Compartilhamento</>)}
                </Button>
              </div>
            )}
          </div>
