...
export function KanbanTimeDisplay({ demandId, canControl = false, forceShow = false, hideIfHasSubdemands = false }: KanbanTimeDisplayProps) {
  ...
  return (
    <div className="@container w-full mb-2">
      <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1 text-xs text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 rounded-md px-2 py-1 min-w-0">
          ...
          >
            {isLoading ? (
              ...
              <>
                ...
                <span className="hidden @[180px]:inline">Pausar</span>
              </>
            ) : (
              <>
                <Play className="h-3 w-3" />
                <span className="hidden @[180px]:inline">Iniciar</span>
              </>
            )}
          </Button>
        )}
