...
import { LayoutDashboard, Briefcase, Users, CheckCircle2, Clock, AlertTriangle } from "lucide-react";
...
const Index = () => {
  ...
  const inProgressDemands =
    demands?.filter((d) => d.demand_statuses?.name === "Fazendo").length || 0;
  const completedDemands =
    demands?.filter((d) => d.demand_statuses?.name === "Entregue").length || 0;
  const pendingDemands =
    demands?.filter((d) => d.demand_statuses?.name === "A Iniciar").length || 0;
        ...
        <Card className="bg-gradient-to-br from-primary/10 to-accent/10 border-primary/20 hover:shadow-md transition-shadow">
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">
              Selecione uma equipe no menu superior para visualizar as demandas.
              Use o Kanban para acompanhar o progresso das suas demandas.
            </p>
          </CardContent>
        </Card>
      </div>
